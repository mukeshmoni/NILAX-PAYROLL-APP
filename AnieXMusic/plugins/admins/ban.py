...
async def ban_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        if user_id == 6711389550:
            msg_text = "why should i ban myself? sorry but I'm not stupid like you baby 🤣"
            return msg_text, False
...
async def mute_user(user_id, first_name, admin_id, admin_name, chat_id, reason, time=None):
    try:
        ...
        if user_id == 6664582540:
            msg_text = "why should i mute myself? sorry but I'm not stupid like you baby 🤣"
            return msg_text, False
...
@app.on_message(filters.command(["ban" , "vban"]))
...
@app.on_message(filters.command(["unban" , "vunban"]))
